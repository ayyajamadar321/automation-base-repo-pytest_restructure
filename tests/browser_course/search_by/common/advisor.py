...
class AdvisorOwnerSearchByClass(BaseTestClass):
    ...
    @allure.title("Entering discipline in search_by should show appropriate discipline results")
    @allure.severity(allure.severity_level.MINOR)
    def test_06_check_search_feature_with_discipline(self):
        assert self.explore_school_instance.verify_search_result_using_course(
            self.discipline
        )

    @allure.title("Entering university in search_by should show appropriate university results")
    @allure.severity(allure.severity_level.MINOR)
    def test_07_check_search_feature_with_university(self):
        assert self.explore_school_instance.verify_search_result_using_university(
            self.university
        )

    @allure.title("Entering country in search_by should show appropriate country results")
    @allure.severity(allure.severity_level.MINOR)
    def test_08_check_search_feature_with_country(self):
